    ...
    for connect in connection:
        try:
            ...
            for table in resultset:
                ...
                print table[0]
                insertcurr.execute(tablesql,cid=connection_id,tname=table[0])
                insertcurr.close()
                                
                # get id (generated by sequence for above table. This value goes into table_id
                print "Fetching Sequence"
                seqid = entryconn.cursor()
                tableid = seqid.execute("select id from PRIME_METADATA_TABLES where connection_id=%s and table_name='%s'" %(connection_id, table[0]))
                tableseqvalue = tableid.fetchone()[0]
                print tableseqvalue
                seqid.close()
                
                # get column name for the table
                colscurr = entryconn.cursor()
                colsList = colscurr.execute("select column_name, data_type from all_tab_columns where table_name='%s'" %(table[0]))
                
                # populate columns in prime_metadata_columns table for a given table
                for cols in colsList:
                    print cols[0],cols[1]
                    colscurr = entryconn.cursor()
                    colscurr.execute(columnsql, tid=tableseqvalue, cname=cols[0], ctype=cols[1])     
